...
#letras del juego
LETRAS = ["L", "U","M","E","I","A","T"] #ver las posiciones para poner las letras
LETRA_CENTRAL = LETRAS[0]
#palabras encontradas
palabras_validas = cargar_palabras_validas("diccionario_sin_acentos.txt", LETRAS, LETRA_CENTRAL)

# Cambiar la inicialización para incluir contadores
palabras_encontradas = {
    letra: {
        'palabras': [],
        'contador': 0,
        'total': 0  # Podemos inicializar esto más tarde
    } for letra in LETRAS
}

# Inicializar los totales (esto podría hacerse al cargar el diccionario)
for letra in LETRAS:
    palabras_encontradas[letra]['total'] = sum(
        1 for palabra in palabras_validas
        if palabra.startswith(letra) and LETRAS[0] in palabra
    )
todas_encontradas = set()  # Para evitar duplicados
...
def aplicar_palabra():
    ...
    letra_central = LETRAS[0]  # La primera letra es la central
    ...
    if (palabra_es_valida(palabra, LETRAS, letra_central) and
        ...
        seleccionados.clear()
        return True  # Indica que la palabra fue aceptada

    return False  # Indica que la palabra no fue aceptada
